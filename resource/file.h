...
#define resource_assign_id( dest, source ) if( NULL != source && strlen( source ) < RESOURCE_PATH_MAX ) { memory_strncpy_ptr( dest, source, RESOURCE_PATH_MAX ); } else { error_printf( "attempted to assign invalid resource: %s", source ); }
