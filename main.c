...
/* === Main Preamble === */
   ...
   struct UNILAYER_STATE* state = NULL;
   ...
   graphics_args.icon_res = icon_unilayer;
...
/* === Subsystem Initialization === */
   ...
   debug_printf( 3,
      UNILAYER_PROJECT_NAME
      " compiled " __DATE__ __TIME__ ", state size is %lu bytes",
      sizeof( struct UNILAYER_STATE ) );
   ...
   g_state_handle = memory_alloc( sizeof( struct UNILAYER_STATE ), 1 );
...
/* === Shutdown === */
   ...
   state = (struct UNILAYER_STATE*)memory_lock( g_state_handle );
   if( NULL == state ) {
   ...
   state = (struct UNILAYER_STATE*)memory_unlock( g_state_handle );
